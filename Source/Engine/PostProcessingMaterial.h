...
private:
	virtual bool LoadEffect(const GameContext& gameContext);
	virtual void LoadEffectVariables() = 0;
	virtual void UpdateEffectVariables(const GameContext& gameContext, RenderTarget* rendertarget) = 0;
	virtual void UnbindEffectVariables(const GameContext& gameContext) = 0;

	void CreateVertexBuffer(const GameContext& gameContext);
	void CreateInputBuffer(const GameContext& gameContext);
