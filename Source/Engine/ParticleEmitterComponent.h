...
class ParticleEmitterComponent : public BaseComponent
{
	...
	ID3DX11EffectScalarVariable* m_pBillboardingVariable;
...
public:
	...
	ParticleEmitterComponent(std::wstring&& assetFile, bool playOnStart = true, int particleCount = 50, bool isBillboard = true);
	...
	void SetVelocity(const DirectX::XMFLOAT3& velocity) { m_Settings.Velocity = velocity; }
	...
	void SetColor(const DirectX::XMFLOAT4& color) { m_Settings.Color = color; }
	void SetAngularVelocity(float angularVel) { m_Settings.AngularVelocity = angularVel; }
