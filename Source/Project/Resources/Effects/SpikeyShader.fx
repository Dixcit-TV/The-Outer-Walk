...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float3 normal, float2 texCoord)
{
    ...
    GS_DATA outData = (GS_DATA)0;
    outData.Position = mul(float4(pos, 1.f), m_MatrixWorldViewProj);
    outData.Normal = normalize(mul(normal, (float3x3)m_MatrixWorld));
