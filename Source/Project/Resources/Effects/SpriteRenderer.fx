...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
    ...
    float scaledWidth = gTextureSize.x * scale.x;
    float scaledHeight = gTextureSize.y * scale.y;
    //VERTEX 1 [LT]
    float2 pivotOffset = float2(-pivot.x * scaledWidth, -pivot.y * scaledHeight);
    CreateVertex(triStream, position, color, texCoord, rotation, rotCosSin, offset, pivotOffset); //Change the color data too!

    //VERTEX 2 [RT]
    pivotOffset = float2((1 - pivot.x) * scaledWidth, -pivot.y * scaledHeight);
    texCoord = float2(1.f, 0.f);
    CreateVertex(triStream, position, color, texCoord, rotation, rotCosSin, offset, pivotOffset); //Change the color data too!

    //VERTEX 3 [LB]
    pivotOffset = float2(-pivot.x * scaledWidth, (1 - pivot.y) * scaledHeight);
    texCoord = float2(0.f, 1.f);
    CreateVertex(triStream, position, color, texCoord, rotation, rotCosSin, offset, pivotOffset); //Change the color data too!

    //VERTEX 4 [RB]
    pivotOffset = float2((1 - pivot.x) * scaledWidth, (1 - pivot.y) * scaledHeight);
